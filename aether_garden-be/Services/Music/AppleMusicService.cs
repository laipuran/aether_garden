...
public class AppleMusicService : IAppleMusicService
{
    ...
    private static readonly Regex PlaylistIdRegex = new("pl\\.[A-Za-z0-9-]+", RegexOptions.Compiled | RegexOptions.IgnoreCase);
    private static readonly Regex StorefrontRegex = new("music\\.apple\\.com\\/([a-z]{2})\\/", RegexOptions.Compiled | RegexOptions.IgnoreCase);
    ...
    public async Task<IReadOnlyList<AppleMusicTrack>> GetFavoriteTracksAsync(CancellationToken cancellationToken = default)
    {
        var ttl = GetCacheTtl();
        var cached = _cache;
        if (cached is not null && cached.Tracks.Count > 0 && DateTimeOffset.UtcNow < cached.ExpiresAt)
        {
    ...
    private async Task<IReadOnlyList<AppleMusicTrack>> FetchTracksAsync(CancellationToken cancellationToken)
    {
        ...
        var apiUrl = $"https://amp-api.music.apple.com/v1/catalog/{storefront}/playlists/{playlistId}?include=tracks";
        ...
        var playlist = await JsonSerializer.DeserializeAsync<PlaylistResponse>(stream, _jsonOptions, cancellationToken);
        ...
        var includedTracks = playlist.Included
            .Where(item => item.Type == "songs" && item.Attributes is not null)
    ...
    private static bool TryParsePlaylist(string playlistUrl, out string storefront, out string playlistId)
    {
        storefront = "cn";
    ...
    private async Task<string?> FetchDeveloperTokenAsync(CancellationToken cancellationToken)
    {
        var client = _httpClientFactory.CreateClient();
        client.DefaultRequestHeaders.UserAgent.ParseAdd("aether-garden-site");
        var response = await client.GetAsync("https://music.apple.com/assets/index-legacy~0484eb127f.js", cancellationToken);
        if (!response.IsSuccessStatusCode)
        {
            return null;
        }
    ...
    private sealed record CachedTracks(IReadOnlyList<AppleMusicTrack> Tracks, DateTimeOffset ExpiresAt);
